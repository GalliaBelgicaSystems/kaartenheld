...
/* Overworld OBJ ramps, generated from assets/palette.txt +
 * tools/palette_slots.json by tools/palette_compiler.py (make manifest).
 * Slots 0-4 are verbatim artist colors; 5-7 are neutral grey. */
#include "cgb_obj_palettes.inc"
...
void ui_init(void)
{
    ...
    /* OBJ slots 0-7: generated (artist ramps 0-4, grey 5-7). */
    OCPS_REG = 0x80;
    for (p = 0; p < 64; p++) {
        OCPD_REG = ((const uint8_t *)cgb_obj_palettes)[p];
    }
